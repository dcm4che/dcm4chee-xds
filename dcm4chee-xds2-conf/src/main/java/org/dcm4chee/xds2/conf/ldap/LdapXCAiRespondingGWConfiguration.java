 ...
 */
public class LdapXCAiRespondingGWConfiguration extends LdapDicomConfigurationExtension {
    private static final String CN_XCA_RESPONDING_GW = "cn=XCAiRespondingGW,";

    @Override
    protected void storeChilds(String deviceDN, Device device) throws NamingException {
        XCAiRespondingGWCfg rspGW = device.getDeviceExtension(XCAiRespondingGWCfg.class);
        if (rspGW != null)
            store(deviceDN, rspGW);
    }

    private void store(String deviceDN, XCAiRespondingGWCfg rspGW)
            throws NamingException {
        config.createSubcontext(CN_XCA_RESPONDING_GW + deviceDN,
                storeTo(rspGW, deviceDN, new BasicAttributes(true)));
    }

    private Attributes storeTo(XCAiRespondingGWCfg rspGW, String deviceDN, Attributes attrs) {
        attrs.put(new BasicAttribute("objectclass", "xcaiRespondingGW"));
    ...
    private void loadFrom(XCAiRespondingGWCfg rspGW, Attributes attrs) throws NamingException {
        ...
        rspGW.setXDSiSourceURLs(LdapUtils.stringArray(attrs.get("xdsiSourceURL")));
    ...
    protected void mergeChilds(Device prev, Device device, String deviceDN)
            throws NamingException {
        XCAiRespondingGWCfg prevRspGW = prev.getDeviceExtension(XCAiRespondingGWCfg.class);
        XCAiRespondingGWCfg rspGW = device.getDeviceExtension(XCAiRespondingGWCfg.class);
        if (rspGW == null) {
            if (prevRspGW != null)
    ...
    private List<ModificationItem> storeDiffs(XCAiRespondingGWCfg prevRspGW, XCAiRespondingGWCfg rspGW,
