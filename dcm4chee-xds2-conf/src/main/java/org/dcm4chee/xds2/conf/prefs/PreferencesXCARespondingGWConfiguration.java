...
public class PreferencesXCARespondingGWConfiguration
    ...
    protected void storeChilds(Device device, Preferences deviceNode) {
        XCARespondigGWCfg rspGW =
                device.getDeviceExtension(XCARespondigGWCfg.class);
        if (rspGW != null)
            storeTo(rspGW, deviceNode.node(NODE_NAME));
    }

    private void storeTo(XCARespondigGWCfg rspGW, Preferences prefs) {
    ...
    private void loadFrom(XCARespondigGWCfg rspGW, Preferences prefs) {
    ...
    protected void mergeChilds(Device prev, Device device, Preferences deviceNode)
        ...
        XCARespondigGWCfg prevRspGW =
                prev.getDeviceExtension(XCARespondigGWCfg.class);
        XCARespondigGWCfg rspGW =
                device.getDeviceExtension(XCARespondigGWCfg.class);
    ...
    private void storeDiffs(Preferences prefs, XCARespondigGWCfg prevRspGW, XCARespondigGWCfg rspGW) {
