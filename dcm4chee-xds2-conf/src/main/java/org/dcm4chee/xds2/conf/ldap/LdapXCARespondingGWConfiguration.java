...
public class LdapXCARespondingGWConfiguration extends LdapDicomConfigurationExtension {
    ...
    protected void storeChilds(String deviceDN, Device device) throws NamingException {
        XCARespondigGWCfg rspGW = device.getDeviceExtension(XCARespondigGWCfg.class);
        if (rspGW != null)
            store(deviceDN, rspGW);
    }

    private void store(String deviceDN, XCARespondigGWCfg rspGW)
            throws NamingException {
        config.createSubcontext(CN_XCA_RESPONDING_GW + deviceDN,
                storeTo(rspGW, deviceDN, new BasicAttributes(true)));
    }

    private Attributes storeTo(XCARespondigGWCfg rspGW, String deviceDN, Attributes attrs) {
    ...
    protected void loadChilds(Device device, String deviceDN)
        ...
        XCARespondigGWCfg rspGW = new XCARespondigGWCfg();
        loadFrom(rspGW, attrs);
        device.addDeviceExtension(rspGW);
    }

    private void loadFrom(XCARespondigGWCfg rspGW, Attributes attrs) throws NamingException {
    ...
    protected void mergeChilds(Device prev, Device device, String deviceDN)
            throws NamingException {
        XCARespondigGWCfg prevRspGW = prev.getDeviceExtension(XCARespondigGWCfg.class);
        XCARespondigGWCfg rspGW = device.getDeviceExtension(XCARespondigGWCfg.class);
        if (rspGW == null) {
            if (prevRspGW != null)
    ...
    private List<ModificationItem> storeDiffs(XCARespondigGWCfg prevRspGW, XCARespondigGWCfg rspGW,
