...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:param name="pid2">PIX-4712^^^TT&amp;1.1.1.1&amp;ISO</xsl:param>
  <xsl:param name="pidNoUID">PIX-4713^^^DCM4CHE-TEST1</xsl:param>
  <xsl:param name="pidWrongUID">PIX-4712^^^&amp;1.0.0.2&amp;ISO</xsl:param>

  <xsl:template match="/hl7">
    <hl7>
      <xsl:variable name="qryDomain"><xsl:value-of select="QPD/field[4]/component[3]/subcomponent[1]" /></xsl:variable>
          ...
          <field>
            <xsl:choose>
            	...
            	<xsl:when test="$qryDomain = '1.1.1.1'">
		            <xsl:call-template name="pid">
		              <xsl:with-param name="cx" select="$pid" />
		            </xsl:call-template><repeat><xsl:call-template name="pid">
			              <xsl:with-param name="cx" select="$pid2" />
			            </xsl:call-template></repeat>
            	</xsl:when>
  ...
  <xsl:template name="pid">
    <xsl:param name="cx"/>
    <xsl:value-of select="substring-before($cx,'^^^')" /><component/><component/><component><xsl:value-of select="substring-before(substring-after($cx,'^^^'),'&amp;')" /><subcomponent><xsl:value-of select="substring-before(substring-after($cx,'&amp;'),'&amp;')" /></subcomponent><subcomponent><xsl:value-of select="substring-after(substring-after($cx,'&amp;'),'&amp;')" /></subcomponent></component>
  </xsl:template>
</xsl:stylesheet>
