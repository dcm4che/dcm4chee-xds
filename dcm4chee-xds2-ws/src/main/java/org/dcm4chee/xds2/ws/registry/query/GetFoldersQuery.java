 ...
 *
 */
public class GetFoldersQuery extends StoredQuery {

    private static Logger log = LoggerFactory.getLogger(GetFoldersQuery.class);

    public GetFoldersQuery(AdhocQueryRequest req, XDSRegistryBean session) throws XDSException {
        super(req, session);
    }
    ...
    public AdhocQueryResponse query() throws XDSException {
        ...
        if ((param = getQueryParam(XDSConstants.QRY_FOLDER_UNIQUE_ID)) != null) {
            if (getQueryParam(XDSConstants.QRY_FOLDER_ENTRY_UUID) != null) {
                throw new XDSException(XDSException.XDS_ERR_REGISTRY_ERROR, 
                        "Both $XDSFolderEntryUUID and $XDSFolderEntryUniqueId are specified!", null);
            }
            String[] values = param.getValues();
            predicate = values.length == 1 ? QXDSFolder.xDSFolder.uniqueId.eq(values[0]) :
                QXDSFolder.xDSFolder.uniqueId.in(values);
        } else if ((param = getQueryParam(XDSConstants.QRY_FOLDER_ENTRY_UUID)) != null) {
            String[] values = param.getValues();
            predicate = values.length == 1 ? QXDSFolder.xDSFolder.id.eq(values[0]) :
                QXDSFolder.xDSFolder.id.in(values);
        }
