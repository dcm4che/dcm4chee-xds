angular.module('AdhocQueryUICtrl', []).controller('AdhocQueryUICtrl', [ '$scope', '$http', 'xdsConstants', 'xdsAdhocQuery', function($scope, $http, xdsConstants, xdsAdhocQuery) {
	...
	$scope.isSearchPid = function() {
		var patIdPattern = /.*&ISO$/;
		return $scope.searchStr.search(patIdPattern) > -1;
	};

	// is query str is a unique id?
	$scope.isSearchUniqueId = function() {
		var uniqueIdPattern = /^[0-9\.]*$/;
		return $scope.searchStr.search(uniqueIdPattern) > -1;
	};

	// is query str is a uuid?
	$scope.isSearchUUID = function() {
		var uuidPattern = /^urn:uuid:.*/;
		return $scope.searchStr.search(uuidPattern) > -1;
	};
