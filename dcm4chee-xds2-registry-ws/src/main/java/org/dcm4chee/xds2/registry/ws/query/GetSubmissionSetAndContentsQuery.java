 ...
 *
 */
public class GetSubmissionSetAndContentsQuery extends StoredQuery {

    private static Logger log = LoggerFactory.getLogger(GetSubmissionSetAndContentsQuery.class);

    public GetSubmissionSetAndContentsQuery(AdhocQueryRequest req, XDSRegistryBean session) throws XDSException {
        super(req, session);
    }
    
    public AdhocQueryResponse query() throws XDSException {
        AdhocQueryResponse rsp = initAdhocQueryResponse();
        XDSSubmissionSet ss = null;
        log.debug("######### session:{}",getSession());
        if (getQueryParam(XDSConstants.QRY_SUBMISSIONSET_UNIQUE_ID) != null) {
            if (getQueryParam(XDSConstants.QRY_SUBMISSIONSET_ENTRY_UUID) != null) {
                throw new XDSException(XDSException.XDS_ERR_REGISTRY_ERROR, 
                        "Both $XDSSubmissionSetEntryUUID and $XDSSubmissionSetUniqueId are specified!", null);
            }
            ss = getSession().getSubmissionSetByUniqueId(getQueryParam(XDSConstants.QRY_SUBMISSIONSET_UNIQUE_ID).getStringValue());
    ...
    public String[] getRequiredParameterNames() {
        return new String[]{XDSConstants.QRY_SUBMISSIONSET_UNIQUE_ID+"|"+XDSConstants.QRY_SUBMISSIONSET_ENTRY_UUID};
    }
